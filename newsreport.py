...
q3 = c.execute("select sub1.date, cast(sub2.err as float)/cast(sub1.raw as float) as percent  from (select date(time) as date, count(*) as raw from log group by date) as sub1, (select date(time) as date, count(*) as err from log where status='404 NOT FOUND' group by date order by date desc) as sub2 where sub1.date=sub2.date;")
result3 = c.fetchall()
